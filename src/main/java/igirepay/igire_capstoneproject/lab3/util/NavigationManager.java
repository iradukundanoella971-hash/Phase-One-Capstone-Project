...
public class NavigationManager {
    ...
    private void wireController(Object controller) {
        if (controller instanceof SplashController splashController) {
            splashController.init(this);
        } else if (controller instanceof LoginController loginController) {
            loginController.init(this, session, appController);
        } else if (controller instanceof RegisterController registerController) {
            registerController.init(this, session, appController);
        } else if (controller instanceof DashboardController dashboardController) {
            dashboardController.init(this, session, appController);
        } else if (controller instanceof TransactionController transactionController) {
            transactionController.init(this, session, appController, transactionMode);
        } else if (controller instanceof ProfileController profileController) {
            profileController.init(this, session, appController);
        }
    }
    ...
    public void goToTransaction(String mode) {
        this.transactionMode = mode == null ? MODE_DEPOSIT : mode;
